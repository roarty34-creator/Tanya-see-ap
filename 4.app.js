const KEY_CATCHES = "tanya_v77_catches";
const KEY_SPOTS = "tanya_v77_spots";
const HARBOUR = { lat:-34.3695, lon:20.8500 };
...
function showTab(id){
  ...
  const btnMap = { add:0, spots:1, drift:2, log:3, stats:4, guide:5, season:6, rules:7 };
  ...
  if(id === "season") renderSeasonGuideBox();
...
function setBaitOptionsForSpecies(){
  ...
  if(merged.includes(current)) baitSelect.value = current;
}
...
function populateLocalAreaOptions(){
  ...
  localArea.innerHTML = `<option value="">Choose saved area...</option>`;
  areas.forEach(a => {
    const opt = document.createElement("option");
    opt.value = a;
    opt.textContent = a;
...
function renderCurrentAreaBox(){
  ...
  const coords = toCoordString(HARBOUR.lat, HARBOUR.lon);
  const area = document.getElementById("localArea").value || "Unknown area";
  box.innerHTML = `Start point: Stilbaai Harbour<br>${coords}<br>Area: ${area}`;
}

function getNearbySpots(){
  const origin = HARBOUR;
  return getAllSpots()
...
function renderNearbySpots(){
  ...
  if(!nearby.length){
    box.innerHTML = `<div class="spotCard mini">No spots found within 20 km of Stilbaai harbour.</div>`;
    return;
  }
  ...
  nearby.forEach(s => {
    ...
    div.innerHTML = `
      ...
      <div class="spotSub">${s.type} • ${s.km.toFixed(2)} km from harbour</div>
      <div class="mini">${coord}</div>
      <div class="row" style="margin-top:8px;">
        <button onclick="useSpotInCatch('${coord}','${encodeURIComponent(s.name)}')">Use in Catch</button>
        <button class="dark" onclick="navigateToSpot('${coord}')">Navigate</button>
        <button class="dark" onclick="openSatelliteMaps('${coord}')">Satellite</button>
      </div>
    `;
    box.appendChild(div);
  });
...
function useSpotInCatch(coordStr, nameEncoded){
...
function getGPSForCatch(){
  navigator.geolocation.getCurrentPosition(async pos => {
    ...
    if(nearest.spot){
      ...
      document.getElementById("localArea").value = "";
...
function openSatelliteMaps(coordStr){
  const c = parseCoordString(coordStr);
  if(!c) return;
  window.open(`https://www.google.com/maps?q=${c.lat},${c.lon}&t=k`, "_blank");
}
...
function saveCatch(){
  ...
  if(!species || !coords || !localArea){
    alert("Choose species, area and coordinates first.");
    return;
  }
...
function renderLog(){
  ...
  catches.forEach(c => {
    ...
    div.innerHTML = `
      ...
      <div class="row" style="margin-top:8px;">
        <button class="dark" onclick="openSatelliteMaps('${c.coords}')">Satellite Nav</button>
      </div>
    `;
    box.appendChild(div);
  });
...
function renderStats(){
  ...
  Object.entries(byArea).sort((a,b) => b[1] - a[1]).slice(0,5).forEach(([k,v]) => {
    html += `${k}: ${v}<br>`;
  });
...
function refreshAllConditions(){
  estimateConditions(HARBOUR.lat, HARBOUR.lon);
  renderCurrentAreaBox();
  renderSpotsDropdowns();
  renderNearbySpots();
}

function renderSeasonGuideBox(){
